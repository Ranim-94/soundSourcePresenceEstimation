...
def main(config):
	...
	#model = torch.load('save/model_2019-02-21_16-12-30.pt')
	
	model.cuda()
	trainer = PresPredTrainer(model, train_dataset, test_dataset=test_dataset, optimizer=optim.Adam, lr=config.lr)
	trainer.train(batch_size=config.batch_size, epochs=config.epochs)
	trainer.test(batch_size=43) # 43 textures of 8*128ms
